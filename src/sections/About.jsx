...
const About = () => {
  return (
    <section id="about" className="max-container mt-[7rem] flex justify-between max-lg:flex-col p-[3rem] max-md:p-3 text-white bg-gray-600 rounded-lg shadow-xl">
      
        <div className="w-1/2 max-lg:w-full rounded-lg p-3 object-contain shadow-xl">

          <h2 className="font-Nunitosans font-bold text-xl text-blue-400">| About</h2>
          
          <h3 className=" text-3xl font-Nunitosans font-bold text-black"> I am a full-stack web developer!</h3>
          <p className="text-lg font-Nunito font-semibold p-0">Proficient in full-stack development, I bring expertise in both front-end and back-end technologies. Adept at translating complex business requirements into efficient, user-friendly applications.</p>
          ...
          <div  className="flex justify-between w-full">
            {stats.map((stat) => (
              <div key={stat.title} className="flex flex-col w-1/3 p-2 text-left">
                ...
                <p className="text-xl font-Nunito font-bold text-blue-400">{stat.title}</p>
              </div>
            ))}
          </div>
          <br />
