    ...
    <section id='projects' className="p-5  max-container w-full flex justify-between flex-col items-center  text-white  mt-[8rem]">
      ...
      <div className="shadow-xl rounded-lg">
        ...
        {projects.map((project) =>(
          <div key={project.title} className='w-full p-4 flex justify-between max-md:flex-col shadow-lg rounded-lg'>

            <div className="w-1/2 max-md:w-full flex flex-col justify-between">
              <h1 className='text-2xl font-Nunitosans font-bold text-black'>Project: {project.title}</h1>
              <p className="text-lg font-Nunito font-semibold text-c p-3">{project.description}</p>
              <a href={project.link} target="_blank" className="flex">
                <img className=" w-9 rounded-lg object-contain" src="git3.png" alt="logo" />
                <h4 className="text-2xl font-Nunitosans font-bold text-green-400 p-3">GitHub Repo</h4>
              </a>
            </div>
            <div className="flex max-contain object-contain w-1/2 max-md:w-full justify-center">
